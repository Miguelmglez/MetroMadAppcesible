...
public class DetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView.setText("Ruta accesible entre las estaciones:");
        textView.append("\n");
        textView.append(ESTACION_ORIGEN.toUpperCase() + " y " + ESTACION_DESTINO.toUpperCase());
        textView.append("\n");
        if (!(((Conexion)rutaFinal.get(0)).getEstacionOrigen().getNombre().equals(ESTACION_ORIGEN) || ((Conexion)rutaFinal.get(0)).getEstacionDestino().getNombre().equals(ESTACION_ORIGEN))){
            textView.append("\n");
            textView.append("La estación " + ESTACION_ORIGEN + " no es accesible. Se ha recalculado la ruta con la estación accesible más cercana.");
            textView.append("\n");
        }
        if (!(((Conexion)rutaFinal.get(rutaFinal.size()-1)).getEstacionOrigen().getNombre().equals(ESTACION_DESTINO) || ((Conexion)rutaFinal.get(rutaFinal.size()-1)).getEstacionDestino().getNombre().equals(ESTACION_DESTINO))){
            textView.append("\n");
            textView.append("La estación " + ESTACION_DESTINO + " no es accesible. Se ha recalculado la ruta con la estación accesible más cercana.");
            textView.append("\n");
        }
        for (int i = 0; i < rutaFinal.size(); i++) {
            ...
            if (i>=1){
                ...
                textView.append("\n");
                textView.append("\n");
                textView.append("Línea: "+ c.getEstacionDestino().getLinea());
            ...
            if (c.getEstacionDestino().getNombre().equals(c.getEstacionOrigen().getNombre())) {
                textView.append("TRANSBORDO EN: " + c.getEstacionDestino().getNombre());
                this.transbordos++;
                ...
                if (reciente.equals(c.getEstacionOrigen().getNombre())){
                    ...
                    textView.append("Línea: "+ c.getEstacionOrigen().getLinea());
        ...
        textView.append("\n");
        textView.append("Número total de estaciones:  " + (rutaFinal.size()));
        textView.append("\n");
        textView.append("Número total de transbordos:  " + transbordos);
    }
